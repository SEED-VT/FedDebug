...
class CartPoleEnv(EnvHandler):
    ...
    def __init__(self, data=None, env_config=None):
        ...
        self.cart_position_min = env_config.get("cart_position_min")
        self.cart_position_max = env_config.get("cart_position_max")
        self.pole_angle_min = env_config.get("pole_angle_min")
        self.pole_angle_max = env_config.get("pole_angle_max")
    ...
    def reset(self):
        ...
        if self.cart_position_min is not None and self.cart_position_max is not None:
            self.state[0] = np.random.uniform(self.cart_position_min, self.cart_position_max)
        if self.pole_angle_min is not None and self.pole_angle_max is not None:
            self.state[2] = np.random.uniform(self.pole_angle_min, self.pole_angle_max)
        self.steps_beyond_done = None
