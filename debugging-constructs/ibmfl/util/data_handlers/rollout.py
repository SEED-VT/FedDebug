...
def create_parser(parser_creator=None):
    ...
    parser.add_argument(
        "--custom_default_policy",
        default="{}",
        type=json.loads,
        help="Algorithm-specific configuration. "
...
def run(args, parser):
    ...
    # update policy with custom model
    if args.custom_default_policy is not None:
        custom_default_policy_name = args.custom_default_policy.get("name")
        custom_default_policy_definition = args.custom_default_policy.get(
            "path")
        if custom_default_policy_definition is not None:
            custom_default_policy_ref = get_class_by_name(custom_default_policy_definition,
                                                          custom_default_policy_name)
            cls = cls.with_updates(
...
def rollout(agent,
    ...
    while keep_going(steps, num_steps, episodes, num_episodes):
        ...
        while not done and keep_going(steps, num_steps, episodes,
            ...
            obs = next_obs
        saver.end_rollout()
        reward_total_episodes += reward_total
        if done:
            episodes += 1
    # return episodes mean reward
    return reward_total_episodes / episodes
